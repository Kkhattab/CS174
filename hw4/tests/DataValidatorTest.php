...
class DataValidatorTest extends UnitTestCase {
    ...
    function testLongLines() {
        ...
        $this->assertIdentical($landing->validate_chart_data("shortline,1,2"), true);
        $this->assertIdentical($landing->validate_chart_data("still valid,42,2.92"), true);
        ...
        // if assertX is not X than the test will fail
        // so for example if assertTrue(x) and x evaluates to false the test will fail
    }
    ...
    function testTooManyLines() {
        $landing = new Controllers\Landing();
        $this->assertIdentical($landing->validate_chart_data("shortline,1,2"), true);
        $this->assertIdentical($landing->validate_chart_data(str_repeat("still valid,42,2.92\n", 10)."valid,1,2"), true);
        $this->assertEqual(
    ...
    function testTuple() {
        $landing = new Controllers\Landing();
        $this->assertIdentical($landing->validate_chart_data("shortline,1,2"), true);
        $this->assertIdentical($landing->validate_chart_data("  some thing, 1,2 "), true);
        $this->assertIdentical($landing->validate_chart_data("fdsa,,91"), true);
    ...
    function testNonNumericValidation() {
        ...
        $this->assertIdentical($landing->validate_chart_data("shortline,1,18.8281"), true);
        $this->assertIdentical($landing->validate_chart_data("  some thing, 5,2 "), true);
        $this->assertIdentical($landing->validate_chart_data("fdsa,,91"), true);
        $this->assertIdentical($landing->validate_chart_data("fdsa,-47,9.1"), true);
